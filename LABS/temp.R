...
gsub( "\\\\", "/", normalizePath( path ) )

normalizePath( "C:\Users\jdlecy\Dropbox\02 - CLASSES\03 - PEDA\Data-Science-Class\DATA" )
...
d$hour <- format( vec, format="%H" )
d$month <- format( vec, format="%b" )
d$day <- format( vec, format="%a" )
d$day365 <- format( vec, format="%j" )
d$week <- format( vec, format="%V" )
...
ggplot( d3, aes( x=week, y=harm, fill=hour )) + 
    geom_area( position='fill' )




d$Collisionmanner <- gsub( "\\(", "", d$Collisionmanner )
d$Collisionmanner <- gsub( "\\)", "", d$Collisionmanner )
d$Collisionmanner <- gsub( "ANGLE Front To SideOther Than Left Turn", "Angle", d$Collisionmanner )
d$Collisionmanner <- gsub( "Sideswipe Same Direction", "Lane Change", d$Collisionmanner )

drop.these <- c("Unknown","10","Rear To Side","Rear To Rear","Sideswipe Opposite Direction","Other")
d$Collisionmanner[ d$Collisionmanner %in% drop.these  ] <- NA

table( d$Collisionmanner )

d$Collisionmanner <- factor(d$Collisionmanner)
...
d3 <- data.frame( table( d$week, d$Collisionmanner ) )
...
d3 <- 
d %>%
group_by( week, Collisionmanner ) %>%
summarize( harm = mean( Totalinjuries > 0 | Totalfatalities > 0 ) )

d3 <- na.omit( d3 )

ggplot( d3, aes( x=as.numeric(week), y=harm, fill= Collisionmanner )) + 
    geom_area( position='fill' ) +
    scale_fill_brewer(  type="qual" )


ggplot( d3, aes( x=week, y=harm, fill= Collisionmanner )) + 
    geom_area( position='fill' ) +
    scale_fill_brewer(  type="qual" )
...
d$Age_Drv1[ d$Age_Drv1 > 99 ] <- NA
d$Age_Drv1[ d$Age_Drv1 < 16 ] <- NA

d$age <- cut( d$Age_Drv1, breaks=c(16,18,25,35,45,55,65,75,100) )
barplot( table(d$age) )

labz <- paste0( c(16,18,25,35,45,55,65,75), "-", c(18,25,35,45,55,65,75,100) )
head( labz )

d$age <- cut( d$Age_Drv1, breaks=c(16,18,25,35,45,55,65,75,100), labels=labz )
barplot( table(d$age) )
...
ggplot( d3, aes( x=as.numeric(as.character(hour)), y=harm, fill=age1 )) + 
    geom_area( position='fill' )
