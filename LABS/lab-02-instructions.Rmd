...
This lab will use [city tax parcel data](https://ds4ps.github.io/Data-Science-Class/DATA/syr_parcels.html) from Syracuse, NY. Each house, business, park or empty lot in the city sits on a parcel of land, and the tax rate for that parcel is determined by the city. All of the characteristics of each parcel is stored in a central database, along with information about the land use classifications, owners, and whether the property is up-to-date on taxes and water bills. Each row represents one tax parcel, and all tax parcels in the city limits are recorded in the dataset. 
...
(2) R wants to make sure you are aware of missing values, so it will return NA (not available) for functions performed on vectors with missing values. Add the 'NA remove' argument to these vectors to ignore missing values: 
