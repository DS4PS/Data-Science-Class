...
This document outlines the instructions for your final project.
...
Create a dashboard with four tabs, one for each part.

Include an option to embed the source code in your dashboard for easy access, and make sure to include the runtime shiny argument. For example:
...
Include the appropriate packages in your app.
...
Your first tab will allow the user to explore traffic accidents in Tempe by time periods. It should include the following elements:
...
* A two-sided slider going from 0 to 23 hours for the time of day.
* Shiny widget check boxes for days of the week. 
* Shiny widget check boxes for the 12 months.
* Shiny widget check boxes for each year of data.

Place the widgets on a separate sidebar on the left-hand side.
...
This tab will allow the user to explore the prevelance and severity of different types of accidents.

You will again create a sidebar for shiny widgets. Organize it into two parts - drivers characteristics, causes, and accident outcomes. Select appropriate widget types.
...
*For time of day, define your own meaningful time periods such as rush hour, work day, school pick-up, etc.).*
...
The main window in this tab should include three tables or graphs:

* Accidents by day of week, one column for counts, one column for count, two for injuries and fatalities.
* Accidents summarized by collision type, one column for count, two for injuries and fatalities.
* Accidents summarized by driver age (use smal number of meaningful categories), one column for count, two for injuries and fatalities.


Add 4 [value boxes](https://rmarkdown.rstudio.com/flexdashboard/using.html#value_boxes) at the top of the page to report statistics for each user query:
...
Include a data table with the raw data that allows the users to filter and download the data they are interested in.
...
Include a description of the project including references for the data source, your team contact info, and the intended use of the dashboard. 

Include the data dictionary in this tab. The city of Tempe provides this file on the data download page. To add it to your RMD file in a nice format, just convert the CSV file into a markdown table or HTML table:
