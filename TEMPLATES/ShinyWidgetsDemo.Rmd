---
title: "Shiny Widgets Demo"
author: "Jesse Lecy"
runtime: shiny
output: html_document
---

Most graphics you create in R will be static graphs and figured used for reports. Sometimes you may wish to leverage dynamic graphics, those that can accept input from the user to filter the data by specific criteria or change arguments to change aesthetics of the graphs. 

The primary package the allows you to create dynamic analysis is called **shiny**, and the input functions are called "shiny widgets". You can take a tour of many of these in the [Shiny Widgets Gallery](https://shiny.rstudio.com/gallery/widget-gallery.html).

This short tutorial introduces you to the basics of shiny apps in R.


## Setup

You will be using the **shiny** library in R, so it needs to be installed before you launch your R Markdown files with embedded shiny functions.

```{r, eval=F}
install.packages( "shiny" )
```

Note, though, that you do not load the library in your RMD file like you do with other packages. Instead, you need to include the "runtime: shiny" argument in your YAML header:
...
You can create a new R Markdown document and select "Shiny" from the list of templates, and it will have these options pre-set.
...
We can embed shiny functions in our RMD reports to make some of the analysis dynamics. You still want the document to look pretty.

Use the argument `echo=F` in your chunk header to hide the code when you render the widget in a markdown file. Your audience will only see the input widget and the rendered tables or plots:
...
In this example we will track the financial performance of five hypothetical stocks over 1,000 days:
...
Now generalize this to allow for users to select the organization ID to highlight a specific stock value.
