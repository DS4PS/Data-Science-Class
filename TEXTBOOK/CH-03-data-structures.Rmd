...
plot( rep(1,4), col="blue", pch=19, cex=6 )   # text used for color argument, use quotes



group <- factor( c("treatment","control","treatment","control") )

plot( rep(1,4), col=group, pch=19, cex=6 )       # object used for color argument, no quotes

```

In the first example we are using a text string as an argument, so it must be enclosed by quotes to specify that it is text.

In the second example the colors are set according to group membership (treatment or control group). Since the argument is now an object (`col=group`), we do not use quotes. 
